    ...
    async saveDataToDatabase(){
      const userEventDay1 = new userEventDay (
        ...
        // const userId = getters.user.id
         console.log('getters.user', this.$store.getters.user.id) //eslint-disable-line
         try {
           const dateFb = await fb.database().ref('user/'+this.$store.getters.user.id).set(userEventDay1) //push
